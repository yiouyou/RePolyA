...
from repolya.azure.chains.prompts.task_definitions import TASK_TYPE2_TASK
        ...
        # Check input data types
            ...
            if isinstance(input_key, str):
                if input_data_type != reference_input and input_data_type != "none":
                    feedback += f"""{name} expects all inputs as {reference_input} or none but the data type of {input_key} is {input_data_type} not {reference_input}. Please find another way.\n"""
            else:
                for res, data_type in zip(input_key, input_data_type):
                    if data_type != reference_input:
                        feedback += f"""{name} expects all inputs as {reference_input} but data type of {res} is {data_type} not {reference_input}. Please find another way.\n
                        """
        elif input_data_type != reference_input:
            feedback += f"""{name} expects all inputs as {reference_input} but the data type of {input_key} is {input_data_type} not {reference_input}. Please find another way.\n"""

        # Check output data types
        if output_data_type != reference_output:
            feedback += f"""{name} should output in {reference_output} data type but it is {output_data_type} not {reference_output}. Please find another way.\n"""
