...
from repolya.paper._paperscraper.get_dumps.utils.chemrxiv.chemrxiv_api import ChemrxivAPI
...
def parse_dump(source_path: str, target_path: str) -> None:
    ...
    logger_paper.info("Done, shutting down")
...
def download_full(save_dir: str, api: Optional[ChemrxivAPI] = None) -> None:
    ...
    for preprint in tqdm(api.all_preprints()):
        ...
        try:
            ...
            logger_paper.warning(f"HTTP API Client error for ID: {preprint_id}")
        except SSLError:
            logger_paper.warning(f"SSLError for ID: {preprint_id}")
