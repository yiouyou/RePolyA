...
from repolya.paper._paperscraper.utils import load_jsonl
...
def save_pdf(paper_metadata: Dict[str, Any], filepath: str) -> None:
    ...
    try:
        ...
        logger_paper.warning(f"Could not download {url}.")
        return
    ...
    try:
        ...
        logger_paper.warning(f"Could not download {pdf_url}.")
        return
...
def save_pdf_from_dump(dump_path: str, pdf_path: str, key_to_save: str = "doi") -> None:
    ...
    for i, paper in enumerate(pbar):
        ...
        if "doi" not in paper.keys() or paper["doi"] is None:
            logger_paper.warning(f"Skipping {paper['title']} since no DOI available.")
            continue
