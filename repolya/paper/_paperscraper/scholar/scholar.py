...
from repolya.paper._paperscraper.utils import dump_papers
...
def get_citations_from_title(title: str) -> int:
    ...
    if len(counts) == 0:
        logger_paper.warning(f"Found no match for {title}.")
        return 0
    if len(counts) > 1:
        logger_paper.warning(f"Found {len(counts)} matches for {title}.")
    return counts[0]
