...
from repolya.coder._gpt_academic._toolbox import trimmed_format_exc, get_reduce_token_percent
from repolya.coder._gpt_academic.request_llm._bridge_chatgpt import model_info, predict_no_ui_long_connection
from repolya._const import DEFAULT_WORKER_NUM
...
def request_gpt_model_in_new_thread_with_ui_alive(
    ...
    logger_coder.debug(f">>> {inputs_show_user}")
    ...
    logger_coder.debug(f"<<< {final_result}")
    return final_result
...
def request_gpt_model_multi_threads_with_very_awesome_ui_and_high_efficiency(
    ...
    if max_workers == -1: # 读取配置文件
        try:
            max_workers = DEFAULT_WORKER_NUM
        except:
            max_workers = 8
        if max_workers <= 0:
            max_workers = 3
